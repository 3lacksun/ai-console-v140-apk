...
import { AccessibilityInfo, Alert, AppState, BackHandler, KeyboardAvoidingView, Platform, SafeAreaView, ScrollView, StyleSheet, Text, TextInput, TouchableOpacity, View, FlatList, useWindowDimensions } from 'react-native';
...
function AIConsoleApp() {
  ...
  const selectWorkspace = (workspaceId) => setConversationState((previous) => { const workspace = previous.workspaces.find((item) => item.id === workspaceId); const nextChat = previous.chats.find((chat) => chat.workspaceId === workspaceId); const nextDocument = (previous.documents || []).find((doc) => doc.workspaceId === workspaceId && doc.status !== 'ARCHIVED'); return { ...previous, activeWorkspaceId: workspace?.id || previous.activeWorkspaceId, activeChatId: nextChat?.id || previous.activeChatId, activeDocumentId: nextDocument?.id || null }; });
  ...
  const handlePinSubmit = async (pin) => { try { if (pinGateMode === 'unlock') { const storedPin = await getLLMSettingsPin(); if (!storedPin || !verifyPinAgainstRecord(pin, storedPin)) return 'Incorrect PIN.'; if (isLegacyPlainPinRecord(storedPin) || pinVerifierNeedsUpgrade(storedPin)) await setLLMSettingsPin(pin); } else { await setLLMSettingsPin(pin); } setPinGateOpen(false); setIsLLMSettingsOpen(true); return ''; } catch (pinError) { return pinError.message || 'Unable to access protected settings.'; } };
  ...
  return <View style={styles.appRoot}><StatusBar style={colorMode === 'light' ? 'dark' : 'light'} /><KeyboardAvoidingView style={styles.keyboardAvoider} behavior={Platform.OS === 'ios' ? 'padding' : 'height'} keyboardVerticalOffset={0}><SafeAreaView style={styles.safe}>
    ...
    {layout === 'compact' && <PrimaryNavigation items={navigationItems} active={primaryDestination} onSelect={(id) => { triggerHaptic(hapticsEnabled); requestPrimaryDestination(id); }} palette={palette} />}
  ...
  <AttachmentSourceSheet visible={isAttachmentSourceOpen} onClose={() => setIsAttachmentSourceOpen(false)} onDocument={handlePickFile} onCamera={handleAddCamera} onGallery={handleAddGallery} returnFocusRef={attachmentTriggerRef} palette={palette} />
