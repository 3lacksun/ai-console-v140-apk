...
export const validateAttachment = (file, { maxBytes = 150 * 1024 * 1024, allowedMimeTypes = [] } = {}) => { if (!file?.name) throw new Error('Attachment name is required.'); if (file.size > maxBytes) throw new Error(`Attachment exceeds the ${maxBytes} byte size ceiling.`); if (allowedMimeTypes.length && file.mimeType && !allowedMimeTypes.includes(file.mimeType)) throw new Error('Unsupported file type.'); return true; };
