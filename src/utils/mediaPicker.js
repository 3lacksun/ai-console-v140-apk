...
const normaliseAsset = async (asset, source) => {
  ...
  if (size > MAX_MEDIA_BYTES) throw new Error('Selected image exceeds the 8 MB per-file ceiling.');
  return {
...
export async function loadImageDataUrl(asset) {
  ...
  if (Math.ceil(base64.length * 3 / 4) > MAX_MEDIA_BYTES) throw new Error('Selected image exceeds the 8 MB per-file ceiling.');
